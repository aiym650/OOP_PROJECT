...
public class Main {
	...
	private static void setupData() throws Exception {
		...
		// ── Users ────────────────────────────────────────────────────────────
		prof = new Teacher("t001", "Asylzhan", "Izbassar", "pass123", Language.EN, 180_000, "CS Dept",
		...
		dean = new Manager("d001", "Bakyt", "Dzhaksybekov", "deanpass", Language.EN, 300_000, "CS Faculty",
				ManagerType.DEAN);
		...
		ResearchPaper p1 = new ResearchPaper("Deep Learning in NLP", "IEEE Journal", 12, LocalDate.of(2022, 3, 15),
				"10.1109/001");
		ResearchPaper p2 = new ResearchPaper("Graph Neural Networks", "ACM Computing", 8, LocalDate.of(2023, 6, 1),
				"10.1145/002");
		ResearchPaper p3 = new ResearchPaper("Transformer Architectures", "Nature AI", 20, LocalDate.of(2021, 1, 10),
				"10.1038/003");
		p1.setCitations(15);
		p2.setCitations(8);
		p3.setCitations(3);
	...
	private static void loginFlow() {
		...
		try {
			...
			if (user instanceof Student s && !(user instanceof GraduateStudent))
				studentMenu(s);
			else if (user instanceof GraduateStudent g)
				gradStudentMenu(g);
	...
	private static void studentMenu(Student s) {
		...
		while (active) {
			...
			case "3" -> {
				...
				String cid = sc.nextLine().trim();
				findCourse(cid).ifPresentOrElse(s::dropCourse, () -> System.out.println("  Course not found."));
			}
			case "4" -> s.viewMarks();
		...
		while (active) {
			...
			case "4" -> {
				...
				if (sv == null)
					...
					System.out.printf("  Supervisor: %s | h-index: %d%n", sv.getFullName(), sv.calculateHIndex());
			}
			case "5" -> {
	...
	private static void teacherMenu(Teacher t) {
		...
		while (active) {
			...
			case "4" -> {
				System.out.print("  Course ID: ");
				findCourse(sc.nextLine().trim()).ifPresentOrElse(t::generateMarkReport,
						() -> System.out.println("  Course not found."));
			}
			case "5" -> {
	...
	private static void adminMenu(Admin a) {
		...
		while (active) {
			...
			System.out.print("  Choice: ");
			switch (sc.nextLine().trim()) {
			case "1" -> allUsers.forEach(u -> System.out.printf("  [%s] %s — %s%n", u.getId(), u.getFullName(),
	...
	private static void managerMenu(Manager m) {
		...
		while (active) {
			...
			case "8" -> {
				...
				int cr;
				try {
					cr = Integer.parseInt(sc.nextLine().trim());
				} catch (NumberFormatException e) {
			...
			case "9" -> {
				...
				System.out.print("  Last name:          ");
				String tln = sc.nextLine().trim();
				System.out.print("  Password:           ");
				String tpw = sc.nextLine().trim();
	...
	private static void listCourses() {
		...
		courses.forEach(c -> System.out.printf("  %-8s %-30s %d credits [%s]%n", c.getCourseId(), c.getName(),
				c.getCredits(), c.getType()));
	}
